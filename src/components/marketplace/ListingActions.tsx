...
interface ListingActionsProps {
  ...
  listingId: string;
  locale: string;
  listingType: 'sell' | 'buy' | 'swap' | 'giveaway';
  listingTitle: string;
...
export function ListingActions({ 
  ...
  const getWhatsAppUrl = () => {
    ...
    // Convert Israeli format (0546093624) to international format (+972546093624)
    if (cleanPhone.startsWith('0')) {
      cleanPhone = '972' + cleanPhone.substring(1);
    }
    
    // Create message based on listing type
    const message = listingType === 'giveaway'
      ? `היי ראיתי שאתה מוסר את ${listingTitle}`
      : `היי ראיתי שאתה מוכר את ${listingTitle}`;
  ...
  return (
    <div className="space-y-3">
        ...
        <>
          {whatsappUrl ? (
            <a
              ...
              className="w-full bg-[#25D366] text-white py-3 px-4 rounded-md font-medium hover:bg-[#20BA5A] transition-colors flex items-center justify-center gap-2"
            >
      ...
      {/* Report Button - visible to all users except owner */}
      {!isOwner && user && (
