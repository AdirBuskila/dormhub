-- Migration: Add business intelligence features (safe version)
-- Run this in your Supabase SQL Editor

-- 1. Safely add columns to products table if they don't exist
DO $$ 
BEGIN
    -- Add total_stock if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'products' AND column_name = 'total_stock'
    ) THEN
        -- Check if 'stock' column exists to rename it
        IF EXISTS (
            SELECT 1 FROM information_schema.columns 
        ...
        ELSE
            -- Neither exists, add it fresh
            ALTER TABLE products ADD COLUMN total_stock INTEGER DEFAULT 0;
        END IF;
    END IF;

    -- Add reserved_stock if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
...
BEGIN
    IF NOT EXISTS (
        ...
        CHECK (source IN ('whatsapp', 'phone', 'portal'));
    END IF;
END $$;
...
-- Add constraint for returns status
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'returns_status_check'
    ) THEN
        ALTER TABLE returns ADD CONSTRAINT returns_status_check 
        CHECK (status IN ('pending', 'inspected', 'restocked', 'refurbish', 'scrap'));
    END IF;
END $$;
...
-- 11. Update existing products data from stock to total_stock
-- (Only if stock column still exists with data)
DO $$ 
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        ...
        -- Copy data from stock to total_stock before we lose it
        UPDATE products SET total_stock = stock WHERE total_stock = 0 OR total_stock IS NULL;
    END IF;
END $$;
